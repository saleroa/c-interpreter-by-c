...
#include <unistd.h> // 用于 read 和 close
...
void next(){
    ...
    while(token = *src){ // 判断 token 的 ascii 码是否为 0， \0 对应 ascii 为 0，表示字符串结束
        // 全是 if ，跳过 空格
...
void global_declaration(){
    ...
    while(token != ';' && token != '}'){
        ...
        if(token == '('){
            ...
            //  ???????????????????????????????????????????????????????????????????????
            current_id[Value] = (int)(text+1);  // 函数地址
...
int main(int argc, char ** argv){
    ...
    printf("basic identifier input ok \n");
